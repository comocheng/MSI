...
#                return array
            ...
            for i,exp in enumerate(exp_dict_list):
                ...
                if parsed_yaml_file_list[i]['moleFractionObservables'][0] != None or parsed_yaml_file_list[i]['concentrationObservables'][0] !=None:
                    ...
                    for observable in Eas:
                        ...
                        for reaction in master_equation_reactions:
                            column = slicing_out_reactions(reaction,observable)
                            array_mapped_to_sensitivity = multiply_by_sensitivites(column,master_equation_sensitivites[reaction]['Ea']*1000)
                            #stub
    ...
    def surrogate_model_molecular_parameters(self,master_equation_sensitivites:dict,
            ...
            for numb,reaction in enumerate(master_equation_reactions):
                ...
                sum_Ea = sum(tempEa)*1000.0*1.987*4.186e3
                #got rid of 4.186
                #stub
                Eas.append(sum_Ea)
...
#                if variable == 'A' or variable == 'n':
#                    temp = np.array(six_parameter_fit_sensitivity_dict[reaction][variable])*np.array(molecular_parameter_updates_dict[reaction])
#                elif variable == 'Ea':
#                    temp = np.array(six_parameter_fit_sensitivity_dict[reaction][variable])*np.array(molecular_parameter_updates_dict[reaction])*(1/1000)*(1/4.186)
...
#                         'Ea': six_paramter_fit_update_dict[reaction]['Ea']*1000*1.984 + nominal_parameter_dictonary[reaction]['Ea'],
#                         'c':six_paramter_fit_update_dict[reaction]['c']*((1000*1.984)**3) + nominal_parameter_dictonary[reaction]['c'],
#                         'd':six_paramter_fit_update_dict[reaction]['d']*((1000*1.984)**-1) + nominal_parameter_dictonary[reaction]['d'],
#                         'f':six_paramter_fit_update_dict[reaction]['f']*((1000*1.984)**-3) + nominal_parameter_dictonary[reaction]['f']}
            #stub
            #start here and start documenting what you're doing 
            print(six_paramter_fit_update_dict[reaction]['Ea'] , 'six parameter fit update dict Ea')
            temp_dict = {'A':np.exp(six_paramter_fit_update_dict[reaction]['A'])*nominal_parameter_dictonary[reaction]['A'],
                         'n':six_paramter_fit_update_dict[reaction]['n']+nominal_parameter_dictonary[reaction]['n'],
